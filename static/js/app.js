...
const _renderWelcomeScreen = function () {
  ...
  if (hasPratica) {
    ...
    infoBoxHtml =
      '<div class="pratica-info-box" style="margin-top:30px;padding:20px;border:2px solid #68c8b2;border-radius:8px;background:rgba(104,200,178,0.1);text-align:left;display:inline-block;">' +
      ...
      (codicePratica
        ? '<tr><td class="field-label" style="padding:4px 12px 4px 0;">Codice</td><td>' +
          codicePratica +
          "</td></tr>"
        : "") +
      (dataIns
        ? '<tr><td class="field-label" style="padding:4px 12px 4px 0;">Data ins.</td><td>' +
          dataIns +
          "</td></tr>"
        : "") +
      '<tr><td class="field-label" style="padding:4px 12px 4px 0;">Nome</td><td>' +
      nomePratica +
      "</td></tr>" +
      '<tr><td class="field-label" style="padding:4px 12px 4px 0;">Richiedente</td><td>' +
      denominazione +
      "</td></tr>" +
      '<tr><td class="field-label" style="padding:4px 12px 4px 0;">Accesso</td><td>' +
      tipoAccesso +
      "</td></tr>" +
      '<tr><td class="field-label" style="padding:4px 12px 4px 0;">Edificio</td><td>' +
      indirizzo +
      "</td></tr>" +
      "</table>" +
      "</div>";
  ...
  const html =
    ...
    '<p style="color:#68c8b2;font-size:1.3rem;font-weight:700;margin-top:12px;text-shadow:0 0 12px rgba(104,200,178,0.3);">versione ' +
    APPVERSION +
    " — " +
    APPDATE +
    "</p>" +
...
const _showHelpWindow = function () {
  ...
  var html =
    ...
    darkFg +
    ";margin-bottom:40px;\">Guida rapida all'uso dell'applicativo</p>" +
    ...
    "<tr><td><strong>Elenco Pratiche</strong></td><td>Mostra l'archivio delle pratiche salvate. Da qui puoi caricare, aprire il riepilogo o eliminare una pratica archiviata.</td></tr>" +
    ...
    "<ul>" +
    "<li><strong>REPORT</strong> — apre una finestra con il report dettagliato, stampabile.</li>" +
    "<li><strong>DATI</strong> — mostra tutti i dati della pratica in formato testo, con pulsante SALVA per scaricare il file completo (include formule e risultati).</li>" +
...
const _setupEventListeners = function () {
  ...
  if (cmdElenco) {
    cmdElenco.addEventListener("click", async function () {
      ...
      for (const p of pratiche) {
        ...
        const modalita =
          (p.dati && p.dati.pratica && p.dati.pratica.modalita_accesso) ||
          "N/D";
        const codice =
          (p.dati && p.dati.pratica && p.dati.pratica.codice) || "";
        const nome =
          p.nome || (p.dati && p.dati.pratica && p.dati.pratica.nome) || "N/D";
        ...
        rowsHtml =
          ...
          nome +
          "</td>" +
          '<td class="td-modalita">' +
          modalita +
          "</td>" +
          '<td class="action-cell">' +
          '<button class="btn-arch btn-arch-load" data-id="' +
          p.id +
          '" title="Carica questa pratica nel wizard">CARICA</button>' +
          '<span class="act-sep">|</span>' +
          '<button class="btn-arch btn-arch-riepilogo" data-id="' +
          p.id +
          '" title="Apre la pratica sulla pagina Riepilogo">RIEPILOGO</button>' +
          '<span class="act-sep">|</span>' +
          '<button class="btn-arch btn-arch-del" data-id="' +
          p.id +
          '" title="Elimina definitivamente questa pratica">ELIMINA</button>' +
          "</td>" +
          "</tr>";
      }
      ...
      winEl.querySelectorAll(".btn-arch-del").forEach(function (btn) {
        btn.addEventListener("click", async function () {
          ...
          const confermato = confirm(
            "Eliminare definitivamente questa pratica?",
          );
  ...
  if (cmdReset) {
    cmdReset.addEventListener("click", async function () {
      const ok = await confirm(
        "Tornare alla pagina iniziale? I dati non salvati andranno persi.",
      );
      if (!ok) return;
      if (!_wizard) return;
      _wizard.reset();
  ...
  if (cmdSalva) {
    cmdSalva.addEventListener("click", async function () {
      ...
      a.href = url;
      a.download =
        "CT30_backup_" + new Date().toISOString().slice(0, 10) + ".json";
      a.click();
      URL.revokeObjectURL(url);
    });
  }
  ...
  if (cmdCarica) {
    cmdCarica.addEventListener("click", function () {
      ...
      input.onchange = async function (e) {
        ...
        if (!file) return;
        if (
          !(await confirm("Caricare sostituir\u00e0 il database. Continuare?"))
        )
          return;
        try {
