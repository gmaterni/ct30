...
export var UaWindowAdm = {
  ...
  setActive(id) {
    // Rimuove la classe active da tutte le finestre
    for (let k in this.ws) {
      if (this.ws[k] && this.ws[k].w) {
        this.ws[k].w.classList.remove("active-window");
      }
    }
    // Aggiunge la classe active alla finestra corrente
    if (this.ws[id] && this.ws[id].w) {
      this.ws[id].w.classList.add("active-window");
    }
  },
  hide(id) {
  ...
  newUaWindow(jqw) {
    let wnd = {
      ...
      show() {
        UaWindowAdm.setActive(this.w.id);
        if (this.firstShow || this.pos == 1 || (this.pos === 0 && this.isVisible === false)) {
          ...
          // Listener per attivazione al click
