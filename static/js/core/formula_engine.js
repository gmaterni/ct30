...
import { RULES, FORMULE_INCENTIVO, PROCEDURA_CONFIG, MAGGIORAZIONI, SOTTO_CATEGORIE_SA, PREMIALITA_CONFIG } from "./normativa.js";
...
const UaFormulaEngine = function() {
    ...
    const _isPAorETS = function(soggettoType) {
        return soggettoType === "Pubblica Amministrazione" || soggettoType === "ETS non economico";
    };

    const _calculatePaymentPlan = function(totalAmount, code, dati, soggettoType) {
        ...
        if (totalAmount <= SOGLIA_UNICA_SOLUZIONE) {
            ...
            plan.documentazioneSemplificata = totalAmount <= (PROCEDURA_CONFIG.SOGLIA_DOCUMENTAZIONE_SEMPLIFICATA || 5000);
            plan.installments.push({ n: 1, amount: parseFloat(totalAmount.toFixed(2)), label: "Unica soluzione" });
            return plan;
        }
    ...
    const _calculateMaggiorazioni = function(contesto) {
        ...
        if (contesto.piccolaImpresa) {
            ...
            result.dettaglio.push({ codice: "piccola_impresa", label: MAGGIORAZIONI.piccola_impresa.label, percentuale: MAGGIORAZIONI.piccola_impresa.percentuale });
        }

        if (contesto.mediaImpresa) {
            result.totale += MAGGIORAZIONI.media_impresa.percentuale;
            result.dettaglio.push({ codice: "media_impresa", label: MAGGIORAZIONI.media_impresa.label, percentuale: MAGGIORAZIONI.media_impresa.percentuale });
        }
    ...
    const _calculatePremialita = function(code, dati, contesto) {
        ...
        if (dati.made_in_eu === "sì" && PREMIALITA_CONFIG.made_in_eu.applicabile_a.includes(code)) {
            result.totale += PREMIALITA_CONFIG.made_in_eu.bonus_perc;
            result.dettaglio.push({ codice: "made_in_eu", label: PREMIALITA_CONFIG.made_in_eu.label, percentuale: PREMIALITA_CONFIG.made_in_eu.bonus_perc });
        }
        ...
        if (code === "II.H" && dati.registro_enea === "sì") {
            ...
            if (dati.ue_production === "sì") {
                ...
                result.dettaglio.push({ codice: "fv_ue_prod", label: fvReg.varianti.ue_prod.label, percentuale: fvReg.varianti.ue_prod.bonus_perc });
            } else if (dati.same_section === "sì") {
                result.totale += fvReg.varianti.same_sec.bonus_perc;
                result.dettaglio.push({ codice: "fv_same_sec", label: fvReg.varianti.same_sec.label, percentuale: fvReg.varianti.same_sec.bonus_perc });
            } else {
                result.totale += fvReg.varianti.iscritti.bonus_perc;
                result.dettaglio.push({ codice: "fv_iscritti", label: fvReg.varianti.iscritti.label, percentuale: fvReg.varianti.iscritti.bonus_perc });
            }
        }

        if (dati.miglioramento_ep_40 === "sì" && PREMIALITA_CONFIG.miglioramento_ep_40.applicabile_a.includes(code)) {
            result.totale += PREMIALITA_CONFIG.miglioramento_ep_40.bonus_perc;
            result.dettaglio.push({ codice: "miglioramento_ep_40", label: PREMIALITA_CONFIG.miglioramento_ep_40.label, percentuale: PREMIALITA_CONFIG.miglioramento_ep_40.bonus_perc });
        }

        if (dati.zona_assistita_a === "sì") {
            result.totale += PREMIALITA_CONFIG.zona_assistita_a.bonus_perc;
            result.dettaglio.push({ codice: "zona_assistita_a", label: PREMIALITA_CONFIG.zona_assistita_a.label, percentuale: PREMIALITA_CONFIG.zona_assistita_a.bonus_perc });
        }

        if (dati.zona_assistita_c === "sì") {
            result.totale += PREMIALITA_CONFIG.zona_assistita_c.bonus_perc;
            result.dettaglio.push({ codice: "zona_assistita_c", label: PREMIALITA_CONFIG.zona_assistita_c.label, percentuale: PREMIALITA_CONFIG.zona_assistita_c.bonus_perc });
        }
    ...
    const _resolvePercentuale = function(code, percDefault, contesto, dati) {
        ...
        const isPAorETS = soggetto === "Pubblica Amministrazione" || soggetto === "ETS non economico";
        ...
        if (soggetto === "Impresa") {
            if (code.startsWith("II.")) {
                base = isMulti
                    ? (intensita.Impresa_multi_Titolo_II || 0.30)
        ...
        if (!isPAorETS && soggetto !== "Impresa") {
            ...
            const hasPercMulti = interventoRules && typeof interventoRules.perc_multi === "number";
            if (hasPercMulti) {
        ...
        let finale = base + maggiorazioneTotale + premialitaTotale;
    ...
    const _executeGenericFormula = function(code, dati, contesto) {
        ...
        if (code === "III.A" || code === "III.B") {
            ...
            if (params.CC === 2.5) {
                warnings.push(code + ": pompa di calore a gas rilevata. Coefficiente CC=2.5 applicato (Manuale Analitico Sez.6).");
            }
            if (code === "III.B") {
                ...
                if (tipoSistema.indexOf("bivalente") !== -1) {
                    ...
                    params.k = potenzaPdc > 35 ? 1.10 : 1.00;
                } else {
                    params.k = 1.25;
                }
            } else {
            ...
            params.eta_s_min_ecodesign = _resolveEtaMinEcodesign(dati.tipologia_pdc, dati.potenza_pdc_kw);
        } else if (code === "II.A") {
            ...
            if (isInterno) {
                warnings.push("Rischio condensa interstiziale: verificare UNI 13788 per isolamento da interno.");
            }
        } else if (code === "II.B") {
            ...
            if (tipo.indexOf("Ampliamento") !== -1) {
                ...
                if (volEsistente > 0 && volNuovo > volEsistente * 0.25) {
                    warnings.push("Ampliamento volumetrico " + volNuovo + " mc eccede il limite del 25% (" + (volEsistente * 0.25) + " mc) previsto per l'intervento II.D.");
                }
            }
        } else if (code === "II.E") {
            ...
            if (potenza > 100) {
                params.cmax_scelto = RULES.interventi["II.G"].varianti["Potenza > 100 kW"]?.cmax_fisso_infrastruttura || 110000;
            } else if (potenza > 50) {
                params.cmax_scelto = RULES.interventi["II.G"].varianti["Potenza > 50 kW e ≤ 100 kW"]?.cmax_fisso_infrastruttura || 60000;
            } else {
                const varKey = tipo.includes("trifase") ? "Punto ricarica Trifase" : "Punto ricarica Monofase";
                const cmaxFisso = RULES.interventi["II.G"].varianti[varKey]?.cmax_fisso || 2400;
                if (potenza > 22) {
                    const cmaxKw = RULES.interventi["II.G"].varianti["Potenza > 22 kW e ≤ 50 kW"]?.cmax_kw || 1200;
            ...
            const scaglioni = RULES.interventi["III.E"].scaglioni_classe[classe] || RULES.interventi["III.E"].scaglioni_classe["A"];
            let fisso = 500;
            for (const s of scaglioni) {
                if (s.fino_a !== undefined && capacita <= s.fino_a) {
                    fisso = s.incentivo_fisso; break;
                }
                if (s.oltre) {
                    fisso = s.incentivo_fisso; break;
                }
            ...
            params.percentuale = ruleIII_C.varianti["Biomassa classe 5 stelle"]?.perc || 0.65;
            ...
            params.ore_funzionamento_annue = parseFloat(dati.ore_funzionamento_annue) || hrZonale || ruleIII_C.ore_funzionamento_default || 1500;
            ...
            const quUtente = parseFloat(dati.produzione_termica_specifica_kwh_mq) || 0;
            ...
            const fattoreTemp = tempCorrezione[dati.temperatura_funzionamento] || 1.00;
            params.Qu = quBase * fattoreTemp;
            if (fattoreTemp !== 1.00) {
                warnings.push("III.D: correzione temperatura " + dati.temperatura_funzionamento + " applicata (fattore " + fattoreTemp + "). Qu corretto da " + quBase.toFixed(0) + " a " + params.Qu.toFixed(0) + " kWh/m².");
            }
            ...
            if (uso === "Solar Cooling") {
                ...
                warnings.push("III.D Solar Cooling: verifica rapporto superficie/potenza assorbitore e DEC.");
            }
            let ci = 0.10;
            const coeffsByUso = ruleIII_D.coefficienti_ci || {};
            const coeffs = coeffsByUso[uso] || coeffsByUso["ACS/Riscaldamento"] || [];
            for (const c of coeffs) {
                if (superficie <= c.sl_max) { ci = c.ci; break; }
            }
            params.Ci = ci;
            params.Sl = superficie;
            const prestazionaleRaw = params.Ci * params.Qu * params.Sl;
            const percentualeCap = Math.min(spesa, superficie * params.cmax) * params.percentuale;
            ...
            for (const f of fasce) {
                if (f.max_kw !== undefined && potenza <= f.max_kw) {
                    cmaxKw = f.cmax_kw; imaxFascia = f.imax; break;
                }
                if (f.min_kw !== undefined && f.max_kw === undefined && potenza > f.min_kw) {
                    cmaxKw = f.cmax_kw; imaxFascia = f.imax; break;
                }
            }
        ...
        if (params.percentuale !== undefined && contesto?.soggetto) {
            params.percentuale = _resolvePercentuale(code, params.percentuale, contesto, dati);
        }
        ...
        if (!finalFormula) {
            return { amount: 0, params: variables, steps: steps, errors: ["Formula base non definita."], warnings: warnings };
        }
        ...
        try {
            ...
            return { amount: 0, params: variables, steps: steps, errors: [`Errore calcolo: ${e.message}`], warnings: warnings };
        }
    ...
    const canCalculate = function(code, dati) {
        ...
        if (code === "II.F" && dati) {
            ...
            if (bac !== "A" && bac !== "B") {
                result.allowed = false;
                result.motivi.push("II.F: la classe BAC deve essere almeno B (minimo per incentivo). Rilevato: " + bac);
            }
        }
        ...
        if (code === "II.B" && dati) {
            ...
            if (valvole === "non presenti") {
                result.allowed = false;
                result.motivi.push("II.B: valvole termostatiche/termoregolazione non presenti. Devono essere già presenti o installate contestualmente.");
            }
        }
        ...
        if (code === "II.E" && dati) {
            ...
            if (eff > 0 && eff < 80) {
                result.allowed = false;
                result.motivi.push("II.E: efficienza luminosa " + eff + " lm/W inferiore al minimo 80 lm/W (Manuale Analitico Sez.4).");
            }
            const criVal = (dati.cri || "").trim();
            if (criVal === "non applicabile") {
                result.allowed = false;
                result.motivi.push("II.E: CRI obbligatorio >80 per interni, >60 per esterni (Manuale Analitico Sez.4).");
            }
        }
        ...
        if (code === "II.D" && dati) {
            ...
            if (tipoNzeb.indexOf("Ampliamento") !== -1) {
                ...
                if (volEsistente > 0 && volNuovo > volEsistente * 0.25) {
                    result.allowed = false;
                    result.motivi.push("II.D: ampliamento volumetrico " + volNuovo + " mc eccede il limite del 25% (" + (volEsistente * 0.25) + " mc).");
                }
            }
        }
        ...
        if ((code === "III.A" || code === "III.B") && dati) {
            ...
            if (!isGas && scop > 0 && scop < 2.6) {
                result.allowed = false;
                result.motivi.push(code + ": SCOP " + scop + " inferiore al minimo 2.6 richiesto per pompe di calore (Manuale Analitico Sez.6).");
            }
        }
        ...
        if (code === "III.B" && dati) {
            ...
            if (tipoSistema.indexOf("ibrido") !== -1 && potenzaCaldaia > 0 && potenzaPdc / potenzaCaldaia > 0.5) {
                result.allowed = false;
                result.motivi.push("III.B: rapporto PdC/caldaia " + (potenzaPdc / potenzaCaldaia).toFixed(2) + " supera il limite 0.5 per sistemi ibridi factory made (Manuale Analitico Sez.6).");
            }
            if (etaSCaldaia > 0) {
                const potCal = parseFloat(dati.potenza_caldaia_kw) || 0;
                if (potCal < 400 && etaSCaldaia <= 90) {
                    result.allowed = false;
                    result.motivi.push("III.B: η_s caldaia " + etaSCaldaia + "% inferiore al minimo 90% per potenza <400 kW (Manuale Analitico Sez.6).");
                }
                if (potCal >= 400 && etaSCaldaia <= 98) {
                    result.allowed = false;
                    result.motivi.push("III.B: η_s caldaia " + etaSCaldaia + "% inferiore al minimo 98% per potenza ≥400 kW (Manuale Analitico Sez.6).");
                }
            }
        }
        ...
        if (code === "III.C" && dati) {
            ...
            if (classe && classe !== "5 stelle") {
                result.allowed = false;
                result.motivi.push("III.C: classe emissiva \"" + classe + "\" non ammessa. Solo classe 5 stelle (Manuale Analitico Sez.6).");
            }
        }
        ...
        if (code === "III.D" && dati) {
            ...
            if (uso === "Solar Cooling") {
                ...
                if (potAss > 0) {
                    ...
                    if (rapporto < 2 || rapporto > 2.75) {
                        result.allowed = false;
                        result.motivi.push("III.D Solar Cooling: rapporto superficie/potenza assorbitore " + rapporto.toFixed(2) + " m²/kW fuori range [2.0–2.75] (Manuale Analitico Sez.6).");
                    }
                }
                if (portAria > 0) {
                    ...
                    if (dec < 8) {
                        result.allowed = false;
                        result.motivi.push("III.D Solar Cooling: DEC " + dec.toFixed(2) + " m²/(1000 m³/h) inferiore al minimo 8 (Manuale Analitico Sez.6).");
                    }
                }
            }
        }
        ...
        if (code === "III.G" && dati) {
            ...
            if (pesVal > 0 && pesVal < 10) {
                result.allowed = false;
                result.motivi.push("III.G: PES " + pesVal + "% inferiore al minimo 10% (Manuale Analitico Sez.6).");
            }
            const potEle = parseFloat(dati.potenza_elettrica) || 0;
            if (potEle > 0 && potEle > 50) {
                result.allowed = false;
                result.motivi.push("III.G: potenza elettrica " + potEle + " kWe superiore al massimo 50 kWe (Manuale Analitico Sez.6).");
            }
        }
        ...
        if (code === "II.H" && dati) {
            ...
            if (potenzaFv > 0) {
                ...
                if (potenzaFv < minKw) {
                    result.allowed = false;
                    result.motivi.push("II.H: potenza FV " + potenzaFv + " kW inferiore al minimo " + minKw + " kW.");
                } else if (potenzaFv > maxKw) {
                    result.allowed = false;
                    result.motivi.push("II.H: potenza FV " + potenzaFv + " kW superiore al massimo " + maxKw + " kW.");
                }
                const trainanteKw = parseFloat(dati._trainante_potenza_kw) || 0;
                if (trainanteKw > 0 && potenzaFv > trainanteKw * 1.05) {
                    result.allowed = false;
                    result.motivi.push("II.H: potenza FV " + potenzaFv + " kW eccede il 105% della potenza della PdC trainante (" + (trainanteKw * 1.05).toFixed(2) + " kW). Sovradimensionamento massimo 5%.");
                }
            }
            if (dati.accumulo_presente === "sì") {
                ...
                if (capAccumulo > 0 && potenzaFv > 0) {
                    ...
                    if (ratio < 0.5) {
                        result.allowed = false;
                        result.motivi.push("II.H: accumulo " + capAccumulo + " kWh insufficiente per " + potenzaFv + " kWp (min 0.5 kWh/kWp, ratio " + ratio.toFixed(2) + ").");
                    } else if (ratio > 2) {
                        result.allowed = false;
                        result.motivi.push("II.H: accumulo " + capAccumulo + " kWh eccede il limite per " + potenzaFv + " kWp (max 2 kWh/kWp, ratio " + ratio.toFixed(2) + ").");
                    }
                }
            }
        }
    ...
    const calculate = function(code, datiTecnici, contesto) {
        ...
        try {
            ...
            if (calculationResult.amount > 0) {
                calculationResult.corrispettivoGSE = _calculateCorrispettivoGSE(calculationResult.amount);
                calculationResult.incentivoNetto = parseFloat(
                    (calculationResult.amount - calculationResult.corrispettivoGSE.importo).toFixed(2)
                );
            }

            if (calculationResult.amount > 0 && calculationResult.errors.length === 0) {
                const soggettoType = contesto?.soggetto || "";
                calculationResult.paymentPlan = _calculatePaymentPlan(calculationResult.amount, code, datiTecnici, soggettoType);
            }
