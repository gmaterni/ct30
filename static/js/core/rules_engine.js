...
const UaRulesEngine = function() {
    ...
    const _isSubjectCompatible = function(subjectType, titleCode) {
        ...
        if (!matriceKey) {
            console.error("_isSubjectCompatible: Soggetto '" + subjectType + "' non mappabile in MATRICE_SA_INTERVENTI.");
            return false;
        }
    ...
    const _checkCatastale = function(categoryCode, subjectType) {
        ...
        if (categoriesLuxury.includes(normalizedCode)) {
            ...
            if (!isPA && !isETSNonEcon) {
                result.isAllowed = false;
                result.reason = "La categoria " + normalizedCode + " è esclusa per il soggetto " + (subjectType || "Privato") + ". Ammessa solo per PA o ETS non economico.";
                return result;
            }
        }
    ...
    const _checkAntimafia = function(importoRichiesto, documentazioneAntimafia) {
        ...
        if (importoRichiesto > soglia && !documentazioneAntimafia) {
            res.success = false;
            res.error = "Soglia ANTIMAFIA: L'importo richiesto (" + importoRichiesto.toLocaleString("it-IT") + "€) supera la soglia di " + soglia.toLocaleString("it-IT") + "€. È obbligatoria la documentazione antimafia (D.Lgs. 159/2011).";
        }
    ...
    const _checkInterventoAmmissibilita = function(subjectType, interventoCode, interventoDati) {
        ...
        var titleCode = code.indexOf("II.") === 0 ? "II" : (code.indexOf("III.") === 0 ? "III" : null);
        if (titleCode && !_isSubjectCompatible(subjectType, titleCode)) {
            var errMsg = "INTERVENTO: Il codice " + code + " (Titolo " + titleCode + ") non è ammesso per il soggetto '" + subjectType + "'.";
            res.success = false;
            res.error = errMsg;
            return res;
        }
        ...
        if (interventoCode === "III.B") {
            ...
            if (tipoSistema.indexOf("ibrido") !== -1 && potenzaCaldaia > 0 && potenzaPdc / potenzaCaldaia > 0.5) {
                res.success = false;
                res.error = "III.B: rapporto PdC/caldaia " + (potenzaPdc / potenzaCaldaia).toFixed(2) + " supera il limite 0.5 per sistemi ibridi factory made (Manuale Analitico Sez.6).";
                return res;
            }
        }
            ...
            if (uso === "Solar Cooling") {
                ...
                if (potAss > 0) {
                    ...
                    if (rapporto < 2 || rapporto > 2.75) {
                        res.success = false;
                        res.error = "III.D Solar Cooling: rapporto superficie/potenza assorbitore " + rapporto.toFixed(2) + " m²/kW fuori range [2.0–2.75] (Manuale Analitico Sez.6).";
                        return res;
                    }
                }
                if (portAria > 0) {
                    ...
                    if (dec < 8) {
                        res.success = false;
                        res.error = "III.D Solar Cooling: DEC " + dec.toFixed(2) + " m²/(1000 m³/h) inferiore al minimo 8 (Manuale Analitico Sez.6).";
                        return res;
                    }
                }
            }
    ...
    const validateAnagrafiche = function(anagraficheData) {
        ...
        if (!anagraficheData) {
            return { success: false, errors: ["Dati anagrafici mancanti."], warnings: [] };
        }
        ...
        if (!prop) {
            errors.push("PROPRIETARIO: Dati anagrafici del proprietario mancanti. Ogni pratica CT3.0 richiede la scheda proprietario.");
        }
        if (!rich) {
            errors.push("RICHIEDENTE/SA: Dati anagrafici del richiedente mancanti. Ogni pratica CT3.0 richiede la scheda richiedente.");
        }
        if (!resp) {
            errors.push("RESPONSABILE/SR: Dati anagrafici del responsabile mancanti. Ogni pratica CT3.0 richiede la scheda responsabile.");
        }
        ...
        if (propUgualeRich && !rich.coincide_con_proprietario) {
            warnings.push("PROPRIETARIO/RICHIEDENTE: CF/P.IVA coincidenti ma flag 'coincide_con_proprietario' non impostato per il richiedente.");
        }
        if (propUgualeResp && !resp.coincide_con_proprietario) {
            warnings.push("PROPRIETARIO/RESPONSABILE: CF/P.IVA coincidenti ma flag 'coincide_con_proprietario' non impostato per il responsabile.");
        }
        if (richUgualeResp && !resp.coincide_con_richiedente) {
            warnings.push("RICHIEDENTE/RESPONSABILE: CF/P.IVA coincidenti ma flag 'coincide_con_richiedente' non impostato per il responsabile.");
        }

        // Se CF diversi, i flag non devono essere attivi
        if (!propUgualeRich && rich.coincide_con_proprietario === true) {
            warnings.push("PROPRIETARIO/RICHIEDENTE: CF/P.IVA diversi ma flag 'coincide_con_proprietario' attivo per il richiedente.");
        }
        if (!propUgualeResp && resp.coincide_con_proprietario === true) {
            warnings.push("PROPRIETARIO/RESPONSABILE: CF/P.IVA diversi ma flag 'coincide_con_proprietario' attivo per il responsabile.");
        }
        if (!richUgualeResp && resp.coincide_con_richiedente === true) {
            warnings.push("RICHIEDENTE/RESPONSABILE: CF/P.IVA diversi ma flag 'coincide_con_richiedente' attivo per il responsabile.");
        }

        // 3. Proprietario ≠ Richiedente → Atto di Assenso obbligatorio
        const propCoincideRich = rich.coincide_con_proprietario === true || prop.coincide_con_richiedente === true;
        if (!propCoincideRich && !propUgualeRich) {
            if (!rich.atto_assenso && !prop.atto_assenso) {
                errors.push("PROPRIETARIO: Se il proprietario è diverso dal Richiedente (SA), è obbligatorio disporre dell'Atto di Assenso del proprietario.");
            }
        }
        ...
        if (tipoRich === "Condominio") {
            if (!rich.verbale_assemblea) {
                errors.push("CONDOMINIO: È obbligatorio il verbale dell'assemblea condominiale che approva l'intervento.");
            }
            if (!rich.tabella_millesimale) {
                errors.push("CONDOMINIO: È obbligatoria la tabella millesimale per la ripartizione delle spese.");
            }
        }

        // 5. Responsabile ≠ Richiedente e SR = ESCO → contratto EPC obbligatorio
        const respCoincideRich = resp.coincide_con_richiedente === true || rich.coincide_con_responsabile === true;
        const tipoResp = resp.tipo_soggetto || "";
        if (!respCoincideRich && tipoResp === "ESCO") {
            if (!resp.contratto_epc) {
                errors.push("RESPONSABILE/SR: Se il SR è ESCO ed è diverso dal Richiedente, è obbligatorio allegare il contratto EPC (UNI CEI EN 17669).");
            }
        }
        ...
        if (tipoResp === "ESCO") {
            if (!resp.certificazione_11352_valida) {
                errors.push("RESPONSABILE/SR: L'ESCO deve possedere certificazione UNI CEI 11352 valida al momento della richiesta.");
            }
        }
        ...
        if (!isSRPA) {
            if (!resp.mandato_incasso) {
                errors.push("RESPONSABILE/SR: Per soggetti non PA è obbligatorio conferire il mandato irrevocabile all'incasso (Art. 13 comma 5).");
            }
        }
    ...
    const validateAccesso = function(modalitaAccesso, datiPratica) {
        ...
        if (!modalitaAccesso) {
            return { success: false, errors: ["Modalità di accesso non specificata."], warnings: [] };
        }

        if (!datiPratica) {
            return { success: false, errors: ["Dati della pratica mancanti per la validazione dell'accesso."], warnings: [] };
        }
        ...
        if (norm === "diretto") {
            ...
            if (datiPratica.dataFineLavori) {
                ...
                if (isNaN(dataFine.getTime())) {
                    ...
                    errors.push("ACCESSO DIRETTO: L'intervento deve essere già concluso. La data fine lavori (" + datiPratica.dataFineLavori + ") è futura.");
                }
            } else {
            ...
            const puoPrenotare = configSoggetto && configSoggetto.prenotazione === true;

            if (!puoPrenotare) {
                errors.push("PRENOTAZIONE: Solo la Pubblica Amministrazione e gli ETS non economici possono accedere alla prenotazione. Il soggetto '" + tipoSoggetto + "' non è abilitato.");
            }
            ...
            if (datiPratica.dataFineLavori) {
                ...
                if (!isNaN(dataFine.getTime()) && dataFine < new Date()) {
                    errors.push("PRENOTAZIONE: L'intervento non deve essere ancora realizzato. La prenotazione è per interventi a preventivo.");
                }
            }

            // CUP obbligatorio
            if (!datiPratica.cup) {
                errors.push("PRENOTAZIONE: Per l'accesso su prenotazione è obbligatorio il Codice Unico di Progetto (CUP).");
            }

            // 50% del budget PA (warning informativo)
            warnings.push("PRENOTAZIONE: L'istanza sarà valutata nei limiti del contingente PA (50% del budget annuo complessivo).");

        } else {
            errors.push("Modalità di accesso '" + modalitaAccesso + "' non riconosciuta. Valori ammessi: 'diretto', 'prenotazione'.");
        }
    ...
    const validateTitoloV = function(datiImpresa, interventi) {
        ...
        if (!datiImpresa) {
            return { success: false, errors: ["Dati impresa mancanti per validazione Titolo V."], warnings: [], intensitaBase: 0, maggiorazioneDimensione: 0 };
        }

        const regoleImpresa = SOTTO_CATEGORIE_SA.impresa.regole;

        // Determina se è multi-intervento
        const isMulti = interventi && Array.isArray(interventi) && interventi.length >= 2;

        // Determina i titoli coinvolti
        const hasTitoloII = interventi && Array.isArray(interventi) && interventi.some(function(c) {
            return c.startsWith("II.");
        });
        const hasTitoloIII = interventi && Array.isArray(interventi) && interventi.some(function(c) {
            return c.startsWith("III.");
        });

        // 1. Richiesta PRELIMINARE obbligatoria prima avvio lavori
        if (!datiImpresa.richiestaPreliminareInviata) {
            errors.push("TITOLO V (R01): Richiesta preliminare obbligatoria prima dell'avvio lavori. Inviare tramite PEC a preliminareimpreseCT3@pec.gse.it.");
        } else {
            // 2. Data richiesta preliminare < data primo impegno
            if (datiImpresa.dataRichiestaPreliminare && datiImpresa.dataPrimoImpegno) {
                ...
                if (!isNaN(dPrel.getTime()) && !isNaN(dImp.getTime())) {
                    if (dPrel >= dImp) {
                        errors.push("TITOLO V (R02): La data della richiesta preliminare (" + datiImpresa.dataRichiestaPreliminare + ") deve essere antecedente alla data del primo impegno di spesa (" + datiImpresa.dataPrimoImpegno + ").");
                    }
                }
            }
        }
        ...
        if (datiImpresa.riduzioneEP !== undefined && datiImpresa.riduzioneEP !== null) {
            const riduzione = parseFloat(datiImpresa.riduzioneEP);
            const sogliaMinima = isMulti ? regoleImpresa.riduzione_ep_multi : regoleImpresa.riduzione_ep_singolo;

            if (!isNaN(riduzione)) {
                if (riduzione < sogliaMinima) {
                    const label = isMulti ? "≥ 20% (multi-intervento)" : "≥ 10% (singolo intervento)";
                    errors.push("TITOLO V (R03): Riduzione domanda energia primaria insufficiente (" + Number(riduzione).toLocaleString("it-IT") + "%). Richiesto " + label + ".");
                }
            } else {
                warnings.push("TITOLO V: Dato 'riduzioneEP' non numerico. Verifica riduzione energia primaria non eseguita.");
            }
        } else {
            warnings.push("TITOLO V: Riduzione domanda energia primaria non fornita. Obbligatorio: ≥ " + (isMulti ? "20" : "10") + "%.");
        }

        // 4. APE pre e post obbligatorio
        if (!datiImpresa.apePreDisponibile) {
            errors.push("TITOLO V (R04): APE pre-intervento obbligatorio per le imprese (Art. 25 D.M. 7/8/2025).");
        }
        if (!datiImpresa.apePostDisponibile) {
            errors.push("TITOLO V (R04): APE post-intervento obbligatorio per le imprese (Art. 25 D.M. 7/8/2025).");
        }

        // 5. Divieto combustibili fossili
        if (datiImpresa.haCombustibiliFossili === true || datiImpresa.haCombustibiliFossili === "si" || datiImpresa.haCombustibiliFossili === "Sì") {
            errors.push("TITOLO V (R05): Vietati apparecchi a combustibili fossili (incluso gas naturale). L'impresa non può installare generatori alimentati a combustibili fossili.");
        }

        // 6. Limite singola impresa: 30 M€/intervento
        if (datiImpresa.costoIntervento !== undefined && datiImpresa.costoIntervento !== null) {
            ...
            if (!isNaN(costo) && costo > limite) {
                errors.push("TITOLO V (R06): Costo intervento (" + costo.toLocaleString("it-IT") + "€) supera il limite di " + Number(regoleImpresa.limite_singolo_intervento_milioni).toLocaleString("it-IT") + " M€ per singola impresa.");
            }
        }
        ...
        if (hasTitoloII && hasTitoloIII) {
            intensitaBase = Math.max(regoleImpresa.intensita_titolo_ii_base, regoleImpresa.intensita_titolo_iii_base);
        } else if (hasTitoloII) {
            intensitaBase = regoleImpresa.intensita_titolo_ii_base;
        ...
        if (isMulti) {
            ...
            if (cumulativo > regoleImpresa.intensita_multi_cumulativo_max) {
                warnings.push("TITOLO V: L'intensità cumulativa (" + (cumulativo * 100).toFixed(0) + "%) supera il massimo del " + (regoleImpresa.intensita_multi_cumulativo_max * 100).toFixed(0) + "% per multi-intervento. Verrà applicato il tetto massimo.");
            }
        }

        // 10. Conservazione documenti: 10 anni (avviso informativo)
        warnings.push("TITOLO V: Obbligo di conservazione documentale per " + regoleImpresa.conservazione_documenti_anni + " anni (aiuti di Stato).");
    ...
    const validateCoincidenzaSRSA = function(tipoSA, tipoSR) {
        if (!tipoSA || !tipoSR) {
            return { success: false, error: "Tipo SA e tipo SR sono obbligatori per la verifica di compatibilità." };
        }

        const saKey = _mapToMatriceKey(tipoSA);
        if (!saKey) {
            return { success: false, error: "Tipo SA '" + tipoSA + "' non riconosciuto nella matrice SA→SR." };
        }

        const srValue = _mapSRToMatriceValue(tipoSR);
        if (!srValue) {
            return { success: false, error: "Tipo SR '" + tipoSR + "' non riconosciuto nella matrice SA→SR." };
        }

        const matriceRow = MATRICE_SA_SR[saKey];
        if (!matriceRow) {
            return { success: false, error: "Nessuna regola di abbinamento SR per SA di tipo '" + tipoSA + "'." };
        }
        ...
        if (!isAmmesso) {
            return {
                success: false,
                error: "ABBINAMENTO SA→SR: Il soggetto '" + tipoSR + "' non è un Responsabile (SR) ammesso per il Richiedente (SA) di tipo '" + tipoSA + "'. SR ammessi: " + ammessi.join(", ") + "."
            };
        }
     ...
     * @private
     */
    const validateInterventiPerSoggetto = function(tipoSA, ambito, codiciIntervento) {
        const errors = [];
        const warnings = [];

        if (!tipoSA) {
            return { success: false, errors: ["Tipo SA non specificato."], warnings: [] };
        }

        if (!codiciIntervento || !Array.isArray(codiciIntervento) || codiciIntervento.length === 0) {
            return { success: false, errors: ["Nessun intervento selezionato."], warnings: [] };
        }

        const matriceKey = _mapToMatriceKey(tipoSA);
        if (!matriceKey) {
            return { success: false, errors: ["Tipo SA '" + tipoSA + "' non mappabile nella matrice SA→Interventi."], warnings: [] };
        }

        const entry = MATRICE_SA_INTERVENTI[matriceKey];
        if (!entry) {
            return { success: false, errors: ["Nessuna regola di ammissibilità interventi per SA '" + tipoSA + "'."], warnings: [] };
        }
        ...
        codiciIntervento.forEach(function(codice) {
            ...
            if (titolo === "II" && !isTitoloIIPermesso) {
                errors.push("INTERVENTO: Il codice " + codice + " (Titolo II) non è ammesso per il soggetto '" + tipoSA + "' (ambito " + (ambito || "non specificato") + ").");
            } else if (titolo === "III" && !isTitoloIIIPermesso) {
                errors.push("INTERVENTO: Il codice " + codice + " (Titolo III) non è ammesso per il soggetto '" + tipoSA + "'.");
            }
        });
        ...
        if (codiciIntervento.indexOf("II.H") !== -1 && !hasIII_A) {
            errors.push("INTERVENTO: II.H (Fotovoltaico+Accumulo) richiede obbligatoriamente III.A (Pompa di calore elettrica).");
        }

        if (codiciIntervento.indexOf("II.G") !== -1 && !hasIII_A) {
            errors.push("INTERVENTO: II.G (Ricarica veicoli) richiede obbligatoriamente III.A (Pompa di calore elettrica).");
        }

        // II.C richiede II.B
        if (codiciIntervento.indexOf("II.C") !== -1 && codiciIntervento.indexOf("II.B") === -1) {
            errors.push("INTERVENTO: II.C (Schermature solari) richiede obbligatoriamente II.B (Sostituzione infissi).");
        }
    ...
    const validateTermini = function(pratica, opts) {
        ...
        if (!pratica) {
            return { success: false, errors: ["Dati pratica mancanti per la validazione termini procedurali."], warnings: [] };
        }
            ...
            if (pratica.data_fine_lavori && pratica.data_richiesta) {
                ...
                if (!isNaN(dtFine.getTime()) && !isNaN(dtRich.getTime())) {
                    ...
                    if (diffGg > tc.accesso_diretto_gg) {
                        errors.push("TERMINI: La richiesta è stata inviata " + diffGg + " giorni dopo la fine lavori. Limite: " + tc.accesso_diretto_gg + " giorni (Art.10 c.1).");
                    }
                }
            }
        }
        ...
        if (mod === "prenotazione" && opts) {
            if (opts.dataAccoglimento && opts.dataAvvioLavori) {
                ...
                if (!isNaN(dtAcc.getTime()) && !isNaN(dtAvv.getTime())) {
                    const diffMesi = (dtAvv.getFullYear() - dtAcc.getFullYear()) * 12 + (dtAvv.getMonth() - dtAcc.getMonth());
                    if (diffMesi > tc.prenotazione_avvio_mesi) {
                        errors.push("TERMINI: L'avvio lavori deve avvenire entro " + tc.prenotazione_avvio_mesi + " mesi dall'accoglimento (Art.14 c.3). Trascorsi " + diffMesi + " mesi.");
                    }
                }
            }
            ...
            if (opts.dataAvvioLavori && pratica.data_fine_lavori) {
                ...
                if (!isNaN(dtAvv.getTime()) && !isNaN(dtFine.getTime())) {
                    ...
                    const termineMesi = isNzeb ? tc.prenotazione_conclusione_nzeb_mesi : tc.prenotazione_conclusione_mesi;
                    const diffMesi = (dtFine.getFullYear() - dtAvv.getFullYear()) * 12 + (dtFine.getMonth() - dtAvv.getMonth());
                    if (diffMesi > termineMesi) {
                        errors.push("TERMINI: La conclusione lavori deve avvenire entro " + termineMesi + " mesi dall'avvio (Art.14 c.4). Trascorsi " + diffMesi + " mesi.");
                    }
                }
            }
        }
        ...
        if (opts && opts.dataTrasmissioneDiagnosi && pratica.data_fine_lavori) {
            ...
            if (!isNaN(dtFine.getTime()) && !isNaN(dtDiag.getTime())) {
                const diffMesi = (dtDiag.getFullYear() - dtFine.getFullYear()) * 12 + (dtDiag.getMonth() - dtFine.getMonth());
                if (diffMesi > tc.diagnosi_trasmissione_mesi) {
                    errors.push("TERMINI: La diagnosi energetica deve essere trasmessa entro " + tc.diagnosi_trasmissione_mesi + " mesi dalla fine lavori (Art.18 c.2). Trascorsi " + diffMesi + " mesi.");
                }
            } else {
                warnings.push("TERMINI: Verifica trasmissione diagnosi non eseguita (date mancanti).");
            }
        }
        ...
        if (opts && opts.variazionePercentuale !== undefined) {
            ...
            if (!isNaN(varPerc) && Math.abs(varPerc) > tc.variazione_soglia_perc) {
                errors.push("TERMINI: Variazione del " + varPerc.toFixed(1) + "% superiore alla soglia del " + tc.variazione_soglia_perc + "%. Richiesta approvazione GSE preventiva (Art.25 c.2).");
            }
        }

        // 6. Divieto reiterazione 1 anno
        if (opts && opts.reiterazione === true) {
            errors.push("TERMINI: La pratica reitera una domanda già respinta/rinunciata prima del termine di " + tc.reiterazione_divieto_anni + " anno(i) dalla comunicazione GSE (Art.30 c.1).");
        }
    ...
    const validateAmmissibilita = function(input) {
        ...
        const soloEsenzioni = input.selectedInterventi
            ? input.selectedInterventi.every(function(c) { return interventiSenzaObbligo.indexOf(c) !== -1; })
            : false;
        ...
        if (!soloEsenzioni && input.potenzaEsistenteKw === undefined && input.buildingStatus === "esistente") {
        ...
        if (validTitles.length === 0 && errors.length === 0) {
            ...
            errors.push("Il soggetto '" + subject + "' non può accedere ai titoli previsti per la categoria '" + category + "'.");
        }
